    ...
    <>
      ...
      <div className="page-root">
        <header className="hero">
          ...
          <div className="accent" aria-hidden></div>
        </header>
      ...
      <style jsx>{`
        ...
        .hero-grid { max-width:1200px; margin:0 auto; display:grid; grid-template-columns: 220px 1fr 340px; gap:18px; align-items:center; }
        .brand { display:flex; align-items:center; gap:12px; }
        .logo { height:56px; width:auto; object-fit:contain; }
        .brand-name{ font-weight:800; font-size:20px; }
        .brand-sub{ color:#6b7788; font-size:12px; }

        .headline{ margin:0; font-size:clamp(28px,5.6vw,56px); line-height:1; font-weight:800; }
        .lead{ color:#64748b; margin-top:8px; }

        .search-row{ display:flex; gap:12px; align-items:center; justify-content:flex-end; }
        .search{ min-width:260px; padding:14px 16px; border-radius:14px; border:1px solid rgba(6,20,40,0.06); background:#fff; box-shadow:0 18px 48px rgba(15,30,60,0.04); }
        .cta{ display:inline-block; padding:12px 16px; border-radius:12px; background:linear-gradient(90deg,#8e44ff,#ff4da6); color:#fff; font-weight:700; text-decoration:none; box-shadow:0 12px 38px rgba(142,68,255,0.12); }

        .accent{ position:absolute; right:28px; top:36px; width:220px; height:160px; border-radius:18px; background:linear-gradient(180deg, rgba(158,76,243,0.14), rgba(255,77,166,0.07)); box-shadow:0 30px 70px rgba(158,76,243,0.06); pointer-events:none; }

        .main{ max-width:1200px; margin:28px auto; display:grid; grid-template-columns: 1fr 320px; gap:24px; padding:0 20px 60px; }
        ...
        .result{ display:flex; justify-content:space-between; align-items:center; padding:16px; border-radius:10px; background:#fbfcff; margin-bottom:14px; }
        ...
        @media (max-width:1100px){
          .hero-grid{ grid-template-columns: 160px 1fr; grid-template-rows:auto auto; gap:12px; }
          .hero-actions{ grid-column:1/-1; display:flex; justify-content:center; margin-top:10px; }
          .accent{ display:none; }
        }
        ...
        @media (max-width:720px){
          ...
          .search-row{ width:100%; justify-content:space-between; }
        }
      `}</style>
    </>
  );
